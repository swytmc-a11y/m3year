...
تدفق الرسائل بكامله على **تطبيق الجوال** (البدء من صاحب مشروع/ممول، الشاشة،
الإشعارات) — لا واجهة رسائل على الويب حاليًا. الأجزاء التي تعيش في هذا
المستودع (وليس `mobile/`):

- **`supabase/functions/notify-new-message/`** — Edge Function (Deno) ترسل
  إشعار Push عبر Expo Push API عند إرسال رسالة. يستدعيها التطبيق مباشرة
  (وليس عبر محفّز قاعدة بيانات) بجلسة المُرسِل؛ تتحقق من الصلاحية عبر RLS ثم
  تستخدم `SUPABASE_SERVICE_ROLE_KEY` **المُوفَّر تلقائيًا** من Supabase داخل
  بيئة تشغيل الدالة نفسها (لم يُخزَّن أو يُمرَّر من قِبلنا) لقراءة رموز جهاز
  المستلم فقط.
- **جدول `push_tokens`** (migration 0010) — تخزين رموز Expo Push، وصول محصور
  بصاحب الرمز فقط عبر RLS.
- **Realtime على `messages`** (migration 0012) — مفعّل عبر
  `supabase_realtime` publication ليتحدّث طرفا المحادثة لحظيًا.
